...
function FullwidthPage(){this.init=function($el){this.$el=$el;const images=this.$el.querySelectorAll('.blocks-gallery-item, .wp-block-image, .wp-block-video');const titles=this.$el.querySelectorAll('.is-style-overlapping-title');for(let i=0;i<images.length;i++){const image=images[i]
...
console.log(words,title.textContent);title.innerHTML=words.map(word=>`<span class="is-style-overlapping-title__word">${word}</span>`).join('')}};}